...
// Response shape: [{ id, title, excerpt, featured_image, slug, acf: { season, category } }]
...
const categories = ["All", "Spring/Summer", "Fall/Winter", "Bridal", "Evening Wear", "Ready-to-Wear"]
    ...
    <main className="pt-20">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredCollections.map((collection) => (
              <Link key={collection.id} href={`/lookbook/${collection.slug}`} className="group">
                ...
                <p className="text-sm text-muted-foreground mb-2">{collection.excerpt}</p>
                <p className="text-xs font-mono uppercase tracking-wider text-accent">{collection.category}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 px-4 bg-muted">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-serif mb-4">Ready to Create Your Custom Piece?</h2>
          <p className="text-muted-foreground mb-8 leading-relaxed">Schedule a consultation to discuss your vision</p>
